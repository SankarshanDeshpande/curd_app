...
function App() {
  ...
  useEffect(() => {
    axios.get('http://localhost:8787/api/v1/users/').then((response)=>{setAllUsers(response.data)})
  ...
  const handleFormSubmit = (e) => {
    ...
    axios
      .post("http://localhost:8787/api/v1/users/", formData)
      .then((response) => {
  ...
  const handleDeleteUser = (id) =>{
    axios.delete(`http://localhost:8787/api/v1/users/${id}`).then((response) => {setMessage("User Deleted Successfully...");}).catch((error) => {setMessage("error while deleting")});
  } 
  ...
  const handleFormUpdate = () =>{
    ...
    axios.put(`http://localhost:8787/api/v1/users/${userId}`,formData).then((response)=>{
      setMessage("User Updated Successfully...")
    }).catch((error) => {setMessage("error while updating")})
  }

  const handleUpdateUser = (id) =>{
    axios.get(`http://localhost:8787/api/v1/users/${id}`).then((response)=>{
